...
To determine why the Internet interface was being preferred, the interface metrics were checked from an elevated PowerShell session using:
...
| Interface | Purpose | Metric |
|---|---|---:|
| AD-Lab | Active Directory network | 25 |
| Internet | Internet connection | 5 |

A lower metric gives an interface higher priority. Therefore, Windows was initially preferring the Internet interface.

The interface priorities were changed:
...
The AD-Lab interface was then given the lower metric and preferred for the Active Directory network.

This was an important troubleshooting exercise because the DNS server address itself was correct. The problem was caused by Windows selecting the wrong network interface when multiple interfaces were active.

In a Service Desk environment, this demonstrates why the effective network configuration and interface selection should be investigated when DNS appears correctly configured but the expected DNS server is not being used.
